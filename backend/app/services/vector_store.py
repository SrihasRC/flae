...
class VectorStoreService:
    ...
    def __init__(
        ...
        if self._client is None and VectorStoreService._cached_client is not None:
            self._client = VectorStoreService._cached_client
    ...
    def _init_client(self) -> ClientAPI:
        ...
        If settings.CHROMA_MODE is 'persistent' (the default):
            Initializes chromadb.PersistentClient directly without network requests.
        If settings.CHROMA_MODE is 'http':
            Initializes chromadb.HttpClient(host=self._host, port=self._port).
        """
        mode = getattr(settings, "CHROMA_MODE", "persistent").lower()
        if mode == "http":
            try:
                client = chromadb.HttpClient(host=self._host, port=self._port)
                client.heartbeat()
                logger.info("ChromaDB connected at %s:%s", self._host, self._port)
                VectorStoreService._cached_client = client
                return client
            except Exception as exc:
                logger.error(
                    "Failed to connect to Chroma HTTP server at %s:%s: %s",
                    self._host,
                    self._port,
                    exc,
                )
            ...
            try:
                ...
                logger.info(
                    "ChromaDB ready (local PersistentClient at %s)", self._persistent_path
                )
                VectorStoreService._cached_client = client
                return client
    ...
    def client(self) -> ClientAPI:
        ...
        if self._client is None:
            if VectorStoreService._cached_client is not None:
                self._client = VectorStoreService._cached_client
            else:
                self._client = self._init_client()
        return self._client
    ...
    def client(self, value: ClientAPI) -> None:
        ...
        self._client = value
        VectorStoreService._cached_client = value
